...
export async function GET(req: NextRequest) { 
    try {
        ...
        const content = await query({
            query: `SELECT * FROM webtoon WHERE ip_title = ?`, 
            values: [ip_title],
          });

          const chapterList = await query({
            query: `SELECT DISTINCT chapter_number FROM webtoon_chapters WHERE ip_title = ? ORDER BY chapter_number ASC`, 
            values: [ip_title],
          });
...
export async function PUT(req: NextRequest) { 
  try {
    ...
    const content = await query({
      query: `UPDATE webtoon SET views = views + 1 WHERE ip_title = ? `, 
      values: [ip_title],
    });
